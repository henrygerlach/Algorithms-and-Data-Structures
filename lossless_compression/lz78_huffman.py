...
def compress(
    ...
    huffman_compressed = huffman_coding.compress(
        data=data, print_statistics=False, return_output=True
    )
...
def compute_statistics(
    ...
    improvement_over_lz78 = (
        (lz78_compressed_size - lz78_huffman_compressed_size) / lz78_compressed_size
    ) * 100
    improvement_over_huffman = (
        (huffman_compressed_size - lz78_huffman_compressed_size)
    ...
    print(
        f"LZ78 + Huffman Compressed Size (bits): "
...
def decompress(
    ...
    huffman_decompressed = huffman_coding.decompress(data=data, return_output=True)

    # LZ78 Decompression Algorithm
    lz78_decompressed = lz78.decompress(
        data=str_to_lz78(huffman_decompressed), return_output=True
    )

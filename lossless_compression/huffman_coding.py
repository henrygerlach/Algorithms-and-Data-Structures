...
def compress(
    ...
    serialized_tree = binary_tree.serialize(
        val_type="char",
        val_func=lambda x: x[0],
        only_leaf_vals=True
    )

    # first NUM_BITS_FOR_TREE_SIZE bits indicate length of serialized tree, 
    # followed by tree and compressed data
    compressed = int_to_bitarray(len(serialized_tree), NUM_BITS_FOR_TREE_SIZE) \
        + serialized_tree + compressed_data
...
def _compute_binary_tree(data: str) -> BinaryTree:
    ...
    data_size = len(data)
    sorted_frequencies = dict(sorted({char: data.count(char)/data_size 
                                for char in set(data)}.items(), 
                                key=lambda item: item[1]))

    # Build Huffman tree using two-queue method
    q1 = Queue([BinaryTree(Node((ch, freq))) 
                for ch, freq in sorted_frequencies.items()])
    q2 = Queue([])
...
def compute_statistics(
    ...
    total_size_bits = (compressed_data_size 
